...
MISSING_FILE_3_FILE = "%s/MissingDrugTrans.txt" % params.TMP_DIR
OUT_DICT_3 = "%s/KEGG3.dict" % params.TMP_DIR
...
def loadSes(start=0, end=-1):
    lines = open(MISSING_FILE_3_FILE).readlines()
    if end > len(lines):
        end = len(lines)
...
def run3(d=None, ses=None, path=OUT_DICT_3):
